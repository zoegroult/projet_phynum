...
def gradient_psi(L_box, N, sc = 1):
    ...
    psi = phi * -2/(3* k * Omega_m0*H0**2)

    psi_tilde = np.fft.fftn(psi)  


    grad_psi_tilde_x = 1j * kx * psi_tilde 
...
def affichage_position(t, L_box, N, sc):
    ...
    H, xedges, yedges = np.histogram2d(x.ravel(), y.ravel(), bins=200, range=[[-L_box/2, L_box/2],[-L_box/2, L_box/2]])     # carte de densité
